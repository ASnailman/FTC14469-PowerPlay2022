...
@Autonomous(name = "SprintAutoRightTarpSLOW", group = "MecanumDrive")
public class SprintAutoRightTarpSLOW extends LinearOpMode {
                ...
                case 6:
                    if (coneLevel == 0) {
                        ...
                        if (BaseControl.GetTaskState() == Task_State.READY || BaseControl.GetTaskState() == Task_State.DONE) {
                            SetAttachmentPositionChoosePower(3000, 1580 + angleAdjustment, 1, 0.4);
                            programOrder++;
                        }
                    }
                    break;
...
//                            ExtendingRail.getCurrentPosition() >= 57) {
                    if (BaseControl.GetTaskState() == Task_State.READY || BaseControl.GetTaskState() == Task_State.DONE) {
                        if (coneLevel == 0) {
...
//                        SetExtendingPositionLowPower(85 + tickAdjustment);
                        SetExtendingPositionLowPower(90 + tickAdjustment);
//                        SetExtendingPositionLowPower(0);
//                        SetAttachmentPositionLowPower(3025, 1212 + angleAdjustment);
                        SetAttachmentPositionChoosePower(3000, 1212 + angleAdjustment, 1, 0.7);
                        //NO TARP
...
//                case 16:
//                    if (ExtendingRail.getCurrentPosition() < 10) {
...
//        RotatingBase.setPower(1);
    ...
    public void SetAttachment_LowPwrRail(int railPos, int basePos) {
        RailControlV2.SetTargetPosition(railPos, -0.4, 0.4);
//        RotatingBase.setTargetPosition(basePos);
//        RotatingBase.setMode(DcMotor.RunMode.RUN_TO_POSITION);
//        RotatingBase.setPower(1);
...
//        RotatingBase.setPower(0.5);
    ...
    public void SetAttachmentPositionChoosePower(int railPos, int basePos, double railPower, double basePower) {
        RailControlV2.SetTargetPosition(railPos, -railPower, railPower);
        BaseControl.SetTargetPosition(basePos, -basePower, basePower);
    }
