...
@Autonomous(name = "SprintAutoRightRed2", group = "MecanumDrive")
public class SprintAutoRightRed2 extends LinearOpMode {
    ...
    BNO055IMU IMU;
    OpenCvWebcam webcam;
    VisionClassAutoRightRed.SignalDeterminationPipeline pipeline;
    ...
    public void runOpMode() {
        ...
        pipeline = new VisionClassAutoRightRed.SignalDeterminationPipeline();
...
//        webcam.setPipeline(new VisionClass.SignalDeterminationPipeline());
        ...
        while (opModeIsActive()) {
            ...
            switch (programOrder) {
                ...
                case 0:
                    if (pipeline.type == VisionClassAutoRightRed.SignalDeterminationPipeline.SignalSleeveType.LocationONE) {
                        posOne = true;
                        posTwo = false;
                        posThree = false;
                        LeftClaw.setPower(1);
                        ...
                        ET.reset();
                    } else if (pipeline.type == VisionClassAutoRightRed.SignalDeterminationPipeline.SignalSleeveType.LocationTWO) {
                        posOne = false;
                        posTwo = true;
                        posThree = false;
                        LeftClaw.setPower(1);
                        RightClaw.setPower(1);
                        ET.reset();
                    } else if (pipeline.type == VisionClassAutoRightRed.SignalDeterminationPipeline.SignalSleeveType.LocationTHREE) {
                        posOne = false;
                        posTwo = false;
                        posThree = true;
                        LeftClaw.setPower(1);
...
//                        SetAttachmentPosition(0, 4953);
                ...
                case 2:
                    if (MechDrive.GetTaskState() == Task_State.INIT ||
                        ...
                        MechDrive.SetTargets(1, 994, 0.36, 1);
                        ET.reset();
                        programOrder++;
                    }
                ...
                case 4:
                    if (MechDrive.GetTaskState() == Task_State.INIT ||
                            MechDrive.GetTaskState() == Task_State.READY ||
                            MechDrive.GetTaskState() == Task_State.DONE) {
                        MechDrive.SetTargets(1, 1180, 0.4, 1);
                        programOrder++;
                    }
                    break;
                ...
                case 9:
                    if (ET.milliseconds() > 700) {
                        if (MechDrive.GetTaskState() == Task_State.READY ||
                                MechDrive.GetTaskState() == Task_State.DONE) {
                            if (repeat == 0) {
                                MechDrive.SetTargets(88, 770, 0.4, 1);
                            }
                            else if (repeat == 1) {
                                MechDrive.SetTargets(88, 750, 0.4, 1);
                            }
                            programOrder++;
                        }
                    }
...
//                              MechDrive.SetTargets(90, 920, 0.4, 1);
                        if (repeat == 0) {
                            MechDrive.SetTargets(90, 310, 0.4, 1);
                            ET.reset();
                        } else if (repeat == 1) {
                            MechDrive.SetTargets(90, 350, 0.4, 1);
                            ET.reset();
                        }
                        programOrder++;
                    }
                ...
                case 15:
                    if (too_close_to_wall) {
                        ...
                        FrontRight.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
                        BackRight.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
                        MechDrive.SetTargets(-88, 50, 0.4, 1);
                    }
                    else if (RotatingBase.getCurrentPosition() < 50 && (MechDrive.GetTaskState() == Task_State.READY ||
                            MechDrive.GetTaskState() == Task_State.DONE)) {
                        RightClaw.setPower(1);
                        LeftClaw.setPower(1);
                        if (RightClaw.getPower() > 0.95 && LeftClaw.getPower() > 0.95) {
                    ...
                    if (RailRight.getCurrentPosition() > 4400) {
                        if (MechDrive.GetTaskState() == Task_State.READY ||
                            ...
                            if (repeat == 0) {
                                ...
                                MechDrive.SetTargets(-92, 1035, 0.4, 1);
                            }
                ...
                case 23:
                    if (ET.milliseconds() > 800) {
                        if (MechDrive.GetTaskState() == Task_State.READY ||
                            ...
                            if (posOne) {
                                MechDrive.SetTargets(-90, 1145, 0.4, 1);
